...
sheets_info = {
    ...
    19: ([4,5,6,7], 's_probsolv', "pso-prof-i", ['name', 'born', 'educ', 'tongue', 'unit']),
...
for tab in choosen_tabs:
    ...
    ed_df.columns = ed_df.columns.str.replace("-_", "_", regex=False)

    if tab == 17:
        ed_df.columns = ed_df.columns.str.replace("_all_", "_", regex=False)
...
default_connection(educ_dfs[4], 's_literacy_age', db_source=db_source)
