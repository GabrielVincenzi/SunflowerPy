...
pop_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.THS_PER.NSA.POP_NC+EMP_NC+SAL_NC+SELF_NC+EMP_DC+SAL_DC+SELF_DC.{countries}", 
...
employment_industry_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.THS_PER.TOTAL+B-E+C+G-I.NSA..{countries}", 
    start_year=start_year,
    end_year=end_year
)
...
inactive_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.PC..Y15-24+Y15-64+Y25-54+Y55-64.{countries}", 
    start_year=start_year,
    end_year=end_year
)
...
unemployment_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.PC..Y15-24+Y15-64+Y25-54+Y55-64.ED0-2+ED3_4+ED5-8.{countries}", 
    start_year=start_year,
    end_year=end_year
)
