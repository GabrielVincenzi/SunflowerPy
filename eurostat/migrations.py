...
asylapp_df = et.get_eurostat_dataset(
    ...
    filters=f"M...T+M+F.FRST+TOTAL.TOTAL+Y_LT14+Y14-17+Y_LT18+Y18-34+Y35-64+Y_GE65.{countries}",
...
asypen_df = et.get_eurostat_dataset(
    ...
    filters=f"M...T+M+F.TOTAL+Y_LT14+Y14-17+Y_LT18+Y18-34+Y35-64+Y_GE65.{countries}",
    start_year=start_year,
    end_year=end_year
)
...
asydec_df = et.get_eurostat_dataset(
    ...
    filters=f"A.PER..T+F+M.TOTAL+Y_LT14+Y14-17+Y_LT18+Y18-34+Y35-64+Y_GE65..{countries}",
    start_year=start_year,
    end_year=end_year
)
...
immig_df = et.get_eurostat_dataset(
    ...
    filters=f"A..COMPLET.TOTAL+Y_LT5+Y5-9+Y10-14+Y_LT15+Y15-19+Y15-64+Y20-24+Y25-29+Y30-34+Y35-39+Y40-44+Y45-49+Y50-54+Y55-59+Y60-64+Y65-69+Y_GE65+Y70-74+Y75-79+Y80-84+Y85-89+Y_GE85+Y90-94+Y95-99+Y_GE100.NR.T+F+M.{countries}",
    start_year=start_year,
    end_year=end_year
)
