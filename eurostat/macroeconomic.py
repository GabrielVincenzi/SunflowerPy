...
gdp_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.CP_MEUR+PC_GDP.NSA.B1GQ+D2+D3+P71+P41+P72+P61+P62+P3.{countries}", 
...
gdp_industry_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.CP_MEUR+PC_GDP.NSA.TOTAL+B-E+C+G-I..{countries}", 
    start_year=start_year,
    end_year=end_year
)
...
debt_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.GD+F2+F31+F32+F3+F4+F41+F42.S13.MIO_EUR+PC_GDP.{countries}", 
    start_year=start_year,
    end_year=end_year
)
...
current_account_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.MIO_EUR+PC_GDP.NSA...WRL_REST.BAL+CRE+DEB.CA+GS+G+S+SA+SB+SC+SD+SE+SF+SG+SI+SL+G1+G2.{countries}", 
    start_year=start_year,
    end_year=end_year
)
...
financial_account_df = et.get_eurostat_dataset(
    ...
    filters=f"Q.MIO_EUR....NET+ASS+LIAB.WRL_REST.NSA.{countries}", 
    start_year=start_year,
    end_year=end_year
)
