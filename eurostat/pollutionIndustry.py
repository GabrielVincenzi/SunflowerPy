...
pollution_amount_df = et.get_eurostat_dataset(
    ...
    filters=f"A.GHG.A+B+C+D+E+F+H+I+J+K+L+M+N+O+P+Q+R+S.KG_HAB+THS_T.{countries}",
...
pollution_intensity_df = et.get_eurostat_dataset(
    ...
    filters=f"A.GHG.A+B+C+D+E+F+H+I+J+K+L+M+N+O+P+Q+R+S.P1+B1G.KG_EUR_CP.{countries}",
    start_year=start_year,
    end_year=end_year
)
...
energy_prod_df = et.get_eurostat_dataset(
    ...
    filters=f"A.TOTAL+CF+RA100+RA200+RA300+RA410+RA420+N9000+X9900.CAP_NET_ELC.PRR_MAIN.MW.{countries}",
    start_year=start_year,
    end_year=end_year
)
...
pollution_intensity_df = clean_stat(pollution_intensity_df, keys=keys, columns_to_pivot=['nace_r2', 'na_item'], prefix='pollintens')
...
descriptions = {
    'pollamount': 'Pollution amount: in Greenhouse gases C02 equivalent',
    'pollintens': 'Pollution intensity: in Greenhouse gases C02 equivalent', 
}

update_json(pollution_df, db_name, pattern, descriptions)
